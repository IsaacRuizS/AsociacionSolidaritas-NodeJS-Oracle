...
-- VISTA: VW_CREDIT_STATUS
CREATE OR REPLACE VIEW VW_CREDIT_STATUS AS
SELECT 
    CS.STATUS_ID,
    CS.DESCRIPTION
FROM CREDIT_STATUS CS;

-- VISTA: VW_LABOR_CONDITION
CREATE OR REPLACE VIEW VW_LABOR_CONDITION AS
SELECT 
    LC.CONDITION_ID,
    LC.DESCRIPTION
FROM LABOR_CONDITION LC;

-- VISTA: VW_WITHDRAWAL
CREATE OR REPLACE VIEW VW_WITHDRAWAL AS
SELECT 
    W.WITHDRAWAL_ID,
    W.SAVING_ID,
    W.AMOUNT,
...
-- VISTA: VW_SAVING_CONTRIBUTION
CREATE OR REPLACE VIEW VW_SAVING_CONTRIBUTION AS
SELECT 
    SC.CONTRIBUTION_ID,
    SC.SAVING_ID,
    SC.AMOUNT,
    SC.DATE_SAVING_CONTRIBUTION
FROM SAVING_CONTRIBUTION SC;

-- VISTA: VW_SAVING_TYPE
CREATE OR REPLACE VIEW VW_SAVING_TYPE AS
SELECT 
    ST.SAVING_TYPE_ID,
    ST.NAME,
    ST.DESCRIPTION
...
-- VISTA: VW_CREDIT_CONTRIBUTION
CREATE OR REPLACE VIEW VW_CREDIT_CONTRIBUTION AS
SELECT 
    CC.CONTRIBUTION_ID,
    CC.CREDIT_ID,
    CC.AMOUNT,
    CC.DATE_CREDIT_CONTRIBUTION
FROM CREDIT_CONTRIBUTION CC;
...
-- CREDIT_STATUS
EXEC SP_CREATE_CREDIT_STATUS('Activo');
EXEC SP_UPDATE_CREDIT_STATUS(1, 'Inactivo');
EXEC SP_DELETE_CREDIT_STATUS(1);

-- LABOR_CONDITION
EXEC SP_CREATE_LABOR_CONDITION('Tiempo Completo');
EXEC SP_UPDATE_LABOR_CONDITION(1, 'Medio Tiempo');
EXEC SP_DELETE_LABOR_CONDITION(1);

-- ROLE
EXEC SP_CREATE_ROLE('Administrador', 'Control total del sistema');
EXEC SP_UPDATE_ROLE(1, 'Usuario', 'Acceso limitado');
EXEC SP_DELETE_ROLE(1);

-- SAVING_TYPE
EXEC SP_CREATE_SAVING_TYPE('Navideño', 'Ahorro exclusivo para diciembre');
EXEC SP_UPDATE_SAVING_TYPE(1, 'Educativo', 'Ahorro para estudios');
EXEC SP_DELETE_SAVING_TYPE(1);

--EXEC SP_LOGIN_ASSOCIATE('correo@ejemplo.com', 'password123', :resultado);
...
SELECT * FROM VW_CREDIT_STATUS;

SELECT * FROM VW_LABOR_CONDITION;

SELECT * FROM VW_WITHDRAWAL;

SELECT * FROM VW_SAVING_CONTRIBUTION;

SELECT * FROM VW_SAVING_TYPE;
